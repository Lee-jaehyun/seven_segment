...
def preprocess(path):
 ...
 #   image = image[int(h*1/5):int(h - (h*1/6)), :]
   ...
   # image_resized = imutils.resize(image=image, height=800, width=700)    # 가변  1200 1000
    ...
    contrast_grayscale = increase_contrast(grayscale, clipLimit=7, size=(5,5))     #(5,5)     (3,3)              # 15
...
def roi_seg(image):
  ...
  #  test_gray = cv2.GaussianBlur(test_gray, (3, 3), 0)
    ...
    test_gray = increase_contrast(test_gray, clipLimit=10, size=(5,5))   # 2
   ...
   # thresh = cv2.threshold(test_gray, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]
    ...
    if 120000< sum(sum(thresh_test)) < 149000:
        _, thresh_2 = cv2.threshold(test_gray, 80, 255, cv2.THRESH_BINARY_INV)
        thresh = thresh_2
    
    elif sum(sum(thresh_test)) < 175000:
        _, thresh_2 = cv2.threshold(test_gray, 80, 255, cv2.THRESH_BINARY_INV)
        thresh = thresh_2
   ...
   # kernel2 = np.ones((28,7), np.uint8)                       # (세로, 가로)     # (28, 4)   #10
    ...
    kernel2 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 35))     # 가로 세로
    kernel3 = cv2.getStructuringElement(cv2.MORPH_RECT, (13, 7))
   ...
   # thresh2 = imutils.resize(image=thresh2, height=800, width=1000)      # 600, 800
    ...
    for c in cnts:
      ...
      #     # x1, y1, w1, h1 = x, y, w, h
        ...
        elif (70 <= w <200) and (300<h<700): # and (4 < h/w < 7):
            digitCnts.append(c)
            cv2.imshow("digit", thresh2[y:y + h, x:x + w])
            cv2.waitKey(0)
...
if __name__ == "__main__":
   ...
   # display = preprocess('../huldang_test.jpeg')
   # display = preprocess('../data/train/체온계/j003.jpg')
    display = preprocess('../data/train/혈당계/p029.jpg')
